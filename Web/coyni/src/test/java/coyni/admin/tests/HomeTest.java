...
import coyni.customer.tests.CustomerProfileTest;
...
public class HomeTest {
	...
	public void testUserDetailsView(String strParams) {
		try {
			...
			homePage.adminUserDetailsPage().accountProfileImagePopup().cropYourImagePopup()
					.verifyCropYourImageHeading();
			homePage.adminUserDetailsPage().accountProfileImagePopup().cropYourImagePopup().navigationComponent()
					.clickBack();
			homePage.adminUserDetailsPage().accountProfileImagePopup().verifyAccountProfileImageHeading();
	...
	public void testUserDetailsChangePasswordWithValid(String strParams) {
		try {
			...
			customerProfileTest.testChangePassword(strParams);
	...
	public void testUserDetailsChangePasswordInvalidCredentials(String strParams) {
		try {
			Map<String, String> data = Runner.getKeywordParameters(strParams);
			CustomerProfileTest customerProfileTest = new CustomerProfileTest();
			customerProfileTest.testChangePasswordInvalidCredentials(strParams);
	...
	public void testUserDetailsEditProfileImage(String strParams) {
		try {
			...
			homePage.adminUserDetailsPage().accountProfileImagePopup().cropYourImagePopup().uploadSelectImage(strParams,
					strParams);
	...
	public void testverifyYourPhoneNumberWithValid(String strParams) {
		try {
			...
			homePage.phoneEmailVerificationComponent().createPasswordPage()
					.fillCreatePassword(data.get("expCreatePassword"));
			homePage.phoneEmailVerificationComponent().createPasswordPage()
					.fillConfirmPassword(data.get("expConfirmPassword"));
