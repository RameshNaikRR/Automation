...
public class LoginTest {
	...
	public void testLoginFieldValidations(String strParams) {
		try {
			...
			loginPage.validateEmail(data.get("email"));
			loginPage.validatePassword(data.get("password"));

		} catch (Exception e) {
			ExtentTestManager.setFailMessageInReport("testFieldValidations exception happend due to " + e);
		}
	}
...
//			loginPage.authyComponent().verifyMessage(data.get("message"));
	...
	@Test
	@Parameters({ "strParams" })
	public void testLoginInvalidData(String strParams) {
		try {
			Map<String, String> data = Runner.getKeywordParameters(strParams);
			loginPage.verifyPageHeading(data.get("loginHeading"));
			loginPage.fillEmail(data.get("email"));
			loginPage.fillPassword(data.get("password"));
			loginPage.clickNext();
			loginPage.toastComponent().verifyToastMessage(data.get("toastMessage"));
		} catch (Exception e) {
			// TODO: handle exception
			ExtentTestManager
					.setFailMessageInReport("Login test with invalid credentials failed due to exception " + e);
